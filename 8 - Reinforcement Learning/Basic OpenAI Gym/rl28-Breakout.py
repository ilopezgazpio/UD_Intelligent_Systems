...
Here in case of breakout we also implemented our own (very simple) clip rewards method.
Reason was - that we wanted to have rewards clipping enabled (to stabilize NN performance) but we also wanted to report real score of a game (from Atari environment) to compare it to Human level from DQN paper.
"""
...
env = wrap_deepmind(env,frame_stack=True,clip_rewards=False,pytorch_img=True)
seed_value = 23
env.seed(seed_value)
...
# Output
directory = './BreakoutVideos/'
env = gym.wrappers.Monitor(env, directory, video_callable=lambda episode_id: episode_id%50==0,force=True)
...
egreedy = 0.9
egreedy_final = 0.01
egreedy_decay = 150000
report_interval = 10
score_to_solve = 32
clip_error = True
normalize_image = True
file2save = 'breakout_save.pth'
...
for i_episode in range(num_episodes):
    ...
    while True:
        ...
        memory.push(state, action, new_state, reward4nn, done)
        qnet_agent.optimize()
