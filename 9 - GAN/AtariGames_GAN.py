...
'''
In this example we train a GAN to generate screenshots of various Atari Games
Modified example from "Deep Reinforcement Learning Hands on" Maxim Lapan
'''
...
class Discriminator(nn.Module):
    ...
    def __init__(self, input_shape):
        ...
        # this pipe converges image into the single number (probability of image being real)
...
if __name__ == "__main__":
    ...
    for batch_v in iterate_batches(envs):
        ...
        '''
        Discriminator is trained by showing it real and fake samples with appropriate labels.
        '''
        dis_optimizer.zero_grad()
        dis_output_true_v = net_discr(batch_v)
        dis_output_fake_v = net_discr(gen_output_v.detach()) # Detach not to accumulate gradients on data passed
        ...
        # train generator
        '''
        This pass teaches the generator how to fool the discriminator and confuse real samples with the generated ones
        '''
        gen_optimizer.zero_grad()
        dis_output_v = net_discr(gen_output_v) # Now we don't stop the gradients :)
        ...
        if iter_no % REPORT_EVERY_ITER == 0:
            log.info("Iter {}: gen_loss={:.3f}, dis_loss={:.3f}".format(iter_no, np.mean(gen_losses), np.mean(dis_losses)))
