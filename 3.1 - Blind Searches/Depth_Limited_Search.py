...
# Task 1
# First of all analyze the DLS procedure (UDAI)
...
blind_dls_tree_agent.blind_search(first_node)

# If solution is found we can explore the solution :)
blind_dls_tree_agent.final_node.depth
blind_dls_tree_agent.final_node.action_history
blind_dls_tree_agent.final_node.observation
blind_dls_tree_agent.final_node.env.render()
# etc

# we can also analyze the search process using the reporting class inside the agent
blind_dls_tree_agent.reporting.log
blind_dls_tree_agent.reporting.plotNumberNodesFrontier()
blind_dls_tree_agent.reporting.plotFrontierMaxDepth()
blind_dls_tree_agent.reporting.plotNodesAddedFrontier(nbins=10)
...
blind_dls_graph_agent.blind_search(first_node)

# If solution is found we can explore the solution :)
blind_dls_graph_agent.final_node.depth
blind_dls_graph_agent.final_node.action_history
blind_dls_graph_agent.final_node.observation
blind_dls_graph_agent.final_node.env.render()

# we can also analyze the search process using the reporting class inside the agent
blind_dls_graph_agent.reporting.log
blind_dls_graph_agent.reporting.plotNumberNodesFrontier()
blind_dls_graph_agent.reporting.plotFrontierMaxDepth()
blind_dls_graph_agent.reporting.plotNodesAddedFrontier(nbins=10)
...
NumberNodesFrontier = pd.concat(
    [
       blind_dls_tree_agent.reporting.log['Frontier'],
       blind_dls_graph_agent.reporting.log['Frontier']
    ] , axis=1
)
...
MaxDepth = pd.concat(
    [
       blind_dls_tree_agent.reporting.log['F.Max.Depth'],
       blind_dls_graph_agent.reporting.log['F.Max.Depth']
    ] , axis=1
)
...
Expanded = pd.concat(
    [
       blind_dls_tree_agent.reporting.log['Expanded'],
       blind_dls_graph_agent.reporting.log['Expanded']
    ] , axis=1
)
...
blind_dls_tree_agent = Blind_DLS_Tree_Agent()
blind_dls_tree_agent.blind_search(first_node, max_steps=10000)
