...
# You can now safely keep on with DFS

# Task 1
# First of all analyze the DFS procedure (UDAI)
...
# If solution is found we can explore the solution :)
blind_dfs_graph_agent.final_node.depth
blind_dfs_graph_agent.final_node.action_history
blind_dfs_graph_agent.final_node.observation
blind_dfs_graph_agent.final_node.env.render()

# we can also analyze the search process using the reporting class inside the agent
blind_dfs_graph_agent.reporting.log
blind_dfs_graph_agent.reporting.plotNumberNodesFrontier()
blind_dfs_graph_agent.reporting.plotFrontierMaxDepth()
blind_dfs_graph_agent.reporting.plotNodesAddedFrontier(nbins=10)
...
NumberNodesFrontier = pd.concat(
    [
       blind_dfs_tree_agent.reporting.log['Frontier'],
       blind_dfs_graph_agent.reporting.log['Frontier']
    ] , axis=1
)
...
MaxDepth = pd.concat(
    [
       blind_dfs_tree_agent.reporting.log['F.Max.Depth'],
       blind_dfs_graph_agent.reporting.log['F.Max.Depth']
    ] , axis=1
)
...
Expanded = pd.concat(
    [
       blind_dfs_tree_agent.reporting.log['Expanded'],
       blind_dfs_graph_agent.reporting.log['Expanded']
    ] , axis=1
)
